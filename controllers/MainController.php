...
class MainController extends Controller
{
    ...
    */
    public function actions() {
        return array(
            ...
            'stream' => array(
                'class' => 'application.modules.mfm-widget.Mfm-widgetStreamAction',
                'mode' => 'normal',
            ),
        );
    }
    ...
    */
    ...
    public function actionCreate(){
        ...
        $_POST = Yii::app()->input->stripClean($_POST);
        $list = new TodoList();
        $list->content->populateByForm();
        $list->title = Yii::app()->request->getParam('title');
        if ($list->validate()) {
            $list->save();
            $this->renderJson(array('wallEntryId' => $list->content->getFirstWallEntryId()));
        } else {
            $this->renderJson(array('errors' => $list->getErrors()), false);
        }
    }
}
